...
grid are _shorter_ than it. Only consider trees in the same row or column; that
...
in each of the four directions. For this tree, this is `_4_` (found by
multiplying `1 * 1 * 2 * 2`).
...
This tree's scenic score is `_8_` (`2 * 2 * 1 * 2`); this is the ideal spot for
the tree house.
