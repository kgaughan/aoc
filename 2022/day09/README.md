...
_tail_ of the rope. If the head moves far enough away from the tail, the tail
...
fact, the head (`H`) and tail (`T`) must _always be touching_ (diagonally
...
example, the tail never moves, and so it only visits `_1_` position. However,
